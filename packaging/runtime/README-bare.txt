Aeterna bare binary
===================

This archive contains only the Aeterna executable, license, and this README. It
contains no Qt runtime, QML imports, platform plugins, or Qt SDK files.

Install an ABI-compatible Qt 6 runtime plus the Qt Quick, Qt Quick Controls, Qt
Quick Layouts, Qt Quick Window, Qt Labs Platform, and platform plugin modules before
running it. The Qt version, compiler ABI, and architecture must match the binary.
Most users should download the portable package instead.
